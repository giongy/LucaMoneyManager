# ─────────────────────────────────────────────────────────────────────────────
# test-annulla.ps1 — audit dell'annullamento: per ogni TIPO di scrittura dell'app esegue il
# gesto, lo annulla dal solo giornale, e confronta l'INTERO database riga per riga con com'era.
#
# Il confronto non è sui saldi né su qualche campo scelto a mano: è il contenuto completo di
# tutte le tabelle journalate. È l'unico modo per vedere i danni che non si vedono — la riga
# figlia sparita, il tag perso, il prezzo medio ricalcolato invece che ripristinato. Una prova
# che guardasse solo i totali li dichiarerebbe tutti superati.
#
#   .\tools\test-annulla.ps1                    # sul DB di progetto
#   .\tools\test-annulla.ps1 -Database prod     # sui dati veri (senza toccarli, vedi sotto)
#   .\tools\test-annulla.ps1 -Verbose           # stampa anche i valori attesi/ottenuti
#   .\tools\test-annulla.ps1 -Keep              # non cancella la copia, per ispezionarla dopo
#
...
# scrive UTF-8 (l'uscita la imposta TestAnnulla su System.out).
...
$copia = Join-Path ([IO.Path]::GetTempPath()) ("mm-annulla-{0}.db" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
Copy-Item $src $copia
$info = Get-Item $src
...
& java --enable-native-access=ALL-UNNAMED --class-path "$classes;$($jar.FullName)" (Join-Path $PSScriptRoot 'TestAnnulla.java') @jargs
$esito = $LASTEXITCODE
