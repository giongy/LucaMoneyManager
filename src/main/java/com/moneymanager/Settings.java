...
/**
 * Impostazioni bootstrap — solo le chiavi necessarie prima che il DB sia aperto.
 * Tutto il resto (appearance.*, backup.*, attachments.dir) vive in app_settings nel DB.
 *
 * File: settings.properties (nella cartella dell'eseguibile)
 */
public class Settings {

    // ─── Chiavi bootstrap (restano su file) ───────────────────────────────────
    ...
    static final Set<String> BOOTSTRAP_KEYS = Set.of(
        DB_PATH, HTTP_PORT, HTTP_ENABLED, AUTOSTART_ENABLED
    );
    ...
    /** Imposta una chiave bootstrap e salva immediatamente su disco. */
    public void set(String key, String value) {
        props.setProperty(key, value);
        save();
    }

    /** Rimuove una chiave dal file e salva (usato dalla migrazione verso il DB). */
    public void remove(String key) {
        props.remove(key);
        save();
    }

    /** Restituisce tutte le chiavi presenti nel file (incluse le non-bootstrap da migrare). */
    ...
    private void save() {
        try {
            ...
            try (OutputStream os = Files.newOutputStream(settingsFile)) {
                props.store(os, "LucaMoneyManager - Bootstrap (db.path, http.*, autostart.*)");
            }
        } catch (IOException e) {
